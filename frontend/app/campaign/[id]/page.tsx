...
const dangerousActions = new Set([
  "cancel_unaccepted_campaign",
  "expire_unaccepted_campaign",
  "request_termination",
  "adjudicate_termination",
]);
...
export default function CampaignPage() {
  ...
  const [terminationCategory, setTerminationCategory] =
    useState("external_hardship");
  ...
  const confirmations: Record<string, string> = {
    approve_counteroffer:
      "Approve every proposed deadline and activate this oath?",
    cancel_unaccepted_campaign:
      "Cancel this unaccepted oath and refund its full escrow?",
    expire_unaccepted_campaign:
      "Expire this invitation and refund its full escrow?",
    finalize_expired_demand:
      "Run the final evidence check? Failure will permanently refund this demand.",
    request_termination:
      "Open a public 48-hour termination case? Submitted statements and evidence cannot be edited or deleted.",
    adjudicate_termination:
      "Ask GenLayer for the final termination ruling now?",
  };
  ...
  async function transact(functionName: string, args: CalldataEncodable[]) {
    ...
    try {
      await ensureNetwork();
      setStage("signing");
      const txHash = await writeClient(address).writeContract({
      ...
      await waitForOutcome(txHash as Hash, {
        onAccepted: () => setStage(usesConsensus ? "evaluating" : "accepted"),
  ...
  if (campaignQuery.isLoading)
    return (
      <div className="glass flex items-center justify-center rounded-2xl p-8 text-center text-muted-foreground">
  ...
  if (campaignQuery.error || !campaign)
    return (
      <div className="glass flex items-center justify-center rounded-2xl p-8 text-center text-destructive">
        This oath could not be loaded. Check the contract configuration and
        campaign ID.
      </div>
    );
  ...
  const isRequester =
    address?.toLowerCase() === termination.requester?.toLowerCase();
  const responseOpen =
    Number(campaign.status) === 3 &&
    Number(termination.status) === 1 &&
    nowSeconds <= Number(termination.response_deadline);
  const adjudicationReady =
    Number(campaign.status) === 3 &&
    [1, 2].includes(Number(termination.status)) &&
    nowSeconds > Number(termination.response_deadline);
    ...
    <>
      <div className="mb-12 flex items-end justify-between gap-8 max-[900px]:flex-col max-[900px]:items-start">
        <div>
          ...
          <p className="mt-3 text-[14px] text-muted-foreground">
            ...
            ↔{" "}
            <span className="inline-flex items-center gap-1">
              {truncateAddress(campaign.kol)}
              <CopyButton value={campaign.kol} label="Copy KOL wallet" />
            </span>
          </p>
        </div>
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-6 items-start max-[900px]:grid-cols-1">
        <section className="grid gap-4">
          ...
          {campaign.demands.map((demand, index) => (
            <GlassCard key={index} className="p-6">
              ...
              <p className="mb-4 text-[14px] leading-[1.5] text-muted-foreground">
                {demand.instructions}
              </p>
              ...
              {demand.evidence_url && (
                <a
                  className="mb-4 block font-mono text-[13px] text-primary hover:underline"
              ...
              {demand.decision?.checked_at &&
                Number(demand.decision.checked_at) > 0 && (
                  <div className="glass-inner rounded-xl border-l-[3px] border-l-primary p-4 mb-4">
                    <strong className="block text-[14px] text-foreground">
                      {demand.decision.passed
                        ? "Verification passed"
                        : "Requirements not yet met"}
                    </strong>
                    <p className="mt-1 text-[13px] text-muted-foreground">
                      {demand.decision.reason}
                    </p>
              ...
              {isKol && [1, 2].includes(Number(demand.status)) && (
                <div className="grid gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      try {
                        return transact("submit_evidence", [
                          id,
                          index,
                          canonicalXUrl(evidence[index] || ""),
                        ]);
                      } catch (caught) {
                        setError(
              ...
              {/* Verify button (Party) */}
              {isParty &&
                Number(demand.status) === 2 &&
                nowSeconds <= Number(demand.active_deadline) && (
                  <button
                    className="inline-flex min-h-10 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-white px-5 py-2 text-[13px] font-medium text-black transition-all duration-200 hover:bg-primary hover:text-white border border-primary w-full"
              ...
              {isParty &&
                [1, 2].includes(Number(demand.status)) &&
                nowSeconds > Number(demand.active_deadline) && (
                  <button
                    className="w-full min-h-10 items-center justify-center gap-2 whitespace-nowrap rounded-full border border-destructive/30 bg-destructive/10 px-5 py-2 text-[13px] font-medium text-destructive transition-all duration-200 hover:bg-destructive/20"
                    onClick={() =>
                      transact("finalize_expired_demand", [id, index])
                    }
                  >
        ...
        <aside className="grid gap-4 max-[900px]:static sticky top-[88px]">
            ...
            <div className="grid gap-2">
              <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                ...
                <strong className="font-mono font-medium text-foreground">
                  {formatGen(campaign.original_escrow)}
                </strong>
              </div>
              <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                <span>Locked</span>
              ...
              <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                ...
                <span className="font-mono text-muted-foreground">
                  {formatGen(campaign.gross_paid)}
                </span>
              </div>
              <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                <span>KOL received</span>
                <span className="font-mono text-muted-foreground">
                  {formatGen(campaign.net_paid)}
                </span>
              </div>
              <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                <span>Platform fee</span>
                <span className="font-mono text-muted-foreground">
                  {formatGen(campaign.fees_paid)}
                </span>
              </div>
              <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                <span>Refunded</span>
                <span className="font-mono text-muted-foreground">
                  {formatGen(campaign.refunded)}
                </span>
              </div>
            </div>
          </GlassCard>
          ...
          {[0, 1].includes(Number(campaign.status)) &&
            nowSeconds > Number(campaign.acceptance_deadline) && (
              <button
                className="inline-flex min-h-10 w-full items-center justify-center gap-2 whitespace-nowrap rounded-full border border-destructive/30 bg-destructive/10 px-5 py-2.5 text-[13px] font-medium text-destructive transition-all duration-200 hover:bg-destructive/20"
          ...
          {isParty && Number(campaign.status) === 2 && (
            <GlassCard className="p-6">
              <h2 className="mb-2 font-display text-[18px] font-semibold text-foreground">
                Request termination
              </h2>
              <p className="mb-4 text-[13px] text-muted-foreground">
                Past payouts remain final. Your statement and public evidence
                are permanent.
              </p>
              <div className="grid gap-3">
                <select
                  ...
                  onChange={(event) =>
                    setTerminationCategory(event.target.value)
                  }
                >
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setTerminationStatement(event.target.value)
                  }
                />
                <textarea
                  className="glass-input w-full rounded-xl px-4 py-2.5 text-foreground text-[14px] min-h-[100px] resize-vertical"
                ...
                <button
                  ...
                  onClick={() =>
                    transact("request_termination", [
                      id,
                      terminationCategory,
                      terminationStatement.trim(),
                      parseUrls(terminationUrls),
                    ])
                  }
                >
            ...
            <GlassCard className="p-6">
              ...
              <div className="grid gap-2 mb-4">
                <div className="flex justify-between gap-4 py-2 text-[14px] text-muted-foreground">
                  <span>Category</span>
                  <strong className="font-mono font-medium text-foreground">
                    {termination.category.replaceAll("_", " ")}
                  </strong>
                </div>
              </div>
              <p className="mb-2 text-[14px] leading-[1.5] text-muted-foreground">
                {termination.statement}
              </p>
              <p className="mb-4 text-[13px] text-muted-foreground">
                Response deadline: {formatDate(termination.response_deadline)}
              </p>
              ...
              {termination.reason && (
                <div className="glass-inner rounded-xl border-l-[3px] border-l-primary p-4 mb-4">
                  <strong className="block text-[14px] text-foreground">
                    Ruling {String(termination.ruling)}
                  </strong>
                  <p className="mt-1 text-[13px] text-muted-foreground">
                    {termination.reason}
                  </p>
              ...
              {isParty && !isRequester && responseOpen && (
                <div className="grid gap-3">
                  <textarea
                    ...
                    onChange={(event) =>
                      setResponseStatement(event.target.value)
                    }
                  />
                  <textarea
                    className="glass-input w-full rounded-xl px-4 py-2.5 text-foreground text-[14px] min-h-[100px] resize-vertical"
                  ...
                  <button
                    ...
                    onClick={() =>
                      transact("respond_to_termination", [
                        id,
                        responseStatement.trim(),
                        parseUrls(responseUrls),
                      ])
                    }
                  >
