  ...
  return (
    <section className="pt-20 sm:pt-28">
      <div className="mb-14">
        ...
        <h2 className="m-0 max-w-[700px] font-display text-[30px] font-semibold leading-[1.13] tracking-[-0.02em] text-bone sm:text-[42px]">
          ...
          <GlassCard key={title} hover className="flex min-h-[240px] flex-col p-6 sm:p-7">
            <span
              className={`mb-12 block font-mono text-[11px] uppercase tracking-[0.16em] ${
                accent === "lavender" ? "text-lavender" : "text-copper"
              }`}
            >
              0{index + 1}
            </span>
            <h3 className="mb-2.5 font-display text-[18px] font-semibold text-bone">
              {title}
            </h3>
            <p className="m-0 text-[14px] leading-[1.5] text-fog">{body}</p>
          </GlassCard>
        ))}
      </div>
    </section>
