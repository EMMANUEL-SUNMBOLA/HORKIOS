...
export function ClosingCTA() {
  return (
    <section className="relative mt-20 overflow-hidden rounded-2xl border border-glass-border bg-black/40 p-8 sm:mt-28 sm:p-12">
      ...
      <div className="relative z-10 grid grid-cols-[1.4fr_0.6fr] items-end gap-20 max-[900px]:grid-cols-1 max-[900px]:gap-10">
        <div>
          ...
          <h2 className="mt-6 font-display text-[30px] font-semibold leading-[1.13] tracking-[-0.02em] text-bone sm:text-[42px]">
            Turn the next campaign<br />into a verifiable oath.
          </h2>
        </div>
        <div>
          <p className="text-[15px] leading-[1.5] text-fog">
            Public testnet GEN has no monetary value. Build, test, and learn how
            programmable agreements behave.
          </p>
          <Link
            className="mt-5 inline-flex min-h-10 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-white px-6 py-3 text-[14px] font-medium text-black transition-all duration-200 hover:bg-bone"
            href="/create"
          >
            Start now <span>↗</span>
          </Link>
